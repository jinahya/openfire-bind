...
import static com.github.jinahya.openfire.persistence.ibatis.mapper.OfConversationMapperTest.acceptMucConversationsPaginated;
...
public class OfMessageArchiveMapperTest
    ...
    private void test(final Long conversationId) {
        acceptOfMessageArchivesPaginated(
                ...
                conversationId,
                ofMessageArchives -> {
                    validate(ofMessageArchives);
                    ofMessageArchives.forEach(ofMessageArchive -> {
                        final OfMessageArchive one = mappedMapper.selectOne01(
                                CATALOG, SCHEMA, ofMessageArchive.getMessageId(),
                                conversationId);
                        validate(one);
                    });
                });
